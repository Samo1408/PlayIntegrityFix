...
# Detect and prefer resetprop-rs over old resetprop
RESETPROP=""
if [ -x "/data/adb/modules/teleinject/resetprop-rs" ]; then
    RESETPROP="/data/adb/modules/teleinject/resetprop-rs"
    ui_print "- Using bundled resetprop-rs"
elif command -v resetprop-rs >/dev/null 2>&1; then
    RESETPROP="resetprop-rs"
    ui_print "- Using system resetprop-rs"
elif command -v resetprop >/dev/null 2>&1; then
    RESETPROP="resetprop"
    ui_print "- Fallback to legacy resetprop"
else
    ui_print "- No resetprop available (optional: place resetprop-rs binary in module dir)"
fi
