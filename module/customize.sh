...
if [ -f "/data/adb/modules/tryinject/pif.prop" ]; then
    ...
    for config in $spoofConfig; do
        grep -q "$config" "/data/adb/modules/tryinject/pif.prop" || continue
        if grep -q "$config=true" "/data/adb/modules/tryinject/pif.prop"; then
...
if [ -f "/data/adb/modules/tryinject/system.prop" ]; then
    cp -af /data/adb/modules/tryinject/system.prop "$MODPATH/system.prop"
fi
