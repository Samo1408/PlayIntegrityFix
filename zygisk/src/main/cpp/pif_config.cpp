...
namespace pif {
    ...
    static const std::array<std::string_view, 22> kTelephonyKeys = {
        ...
        "FINGERPRINT", "MODEL", "PRODUCT", "BRAND", "DEVICE",
        "MANUFACTURER", "SECURITY_PATCH", "DEVICE_INITIAL_SDK_INT"
    };
    ...
    Config parseConfig(std::string_view content) {
        ...
        if (auto v = take("spoofBuild"); !v.empty()) config.spoofBuild = parseBool(v);
        if (auto v = take("spoofProps"); !v.empty()) config.spoofProps = parseBool(v);
        if (auto v = take("spoofProvider"); !v.empty()) config.spoofProvider = parseBool(v);
        if (auto v = take("spoofSignature"); !v.empty()) config.spoofSignature = parseBool(v);
        if (auto v = take("DEBUG"); !v.empty()) config.debug = parseBool(v);

        if (auto v = take("FINGERPRINT"); !v.empty()) config.fingerprint = v;
        if (auto v = take("MODEL"); !v.empty()) config.model = v;
        if (auto v = take("PRODUCT"); !v.empty()) config.product = v;
        if (auto v = take("BRAND"); !v.empty()) config.brand = v;
        if (auto v = take("DEVICE"); !v.empty()) config.device = v;
        if (auto v = take("MANUFACTURER"); !v.empty()) config.manufacturer = v;
        if (auto v = take("SECURITY_PATCH"); !v.empty()) config.securityPatch = v;
        if (auto v = take("DEVICE_INITIAL_SDK_INT"); !v.empty()) config.deviceInitialSdkInt = v;
    ...
    bool writeConfig(int fd, const Config &config) {
        ...
        ok = ok && writeExact(fd, &config.spoofBuild, sizeof(config.spoofBuild));
        ...
        ok = ok && writeString(fd, config.fingerprint);
        ok = ok && writeString(fd, config.model);
        ok = ok && writeString(fd, config.product);
        ok = ok && writeString(fd, config.brand);
        ok = ok && writeString(fd, config.device);
        ok = ok && writeString(fd, config.manufacturer);
    ...
    bool readConfig(int fd, Config &config) {
        ...
        ok = ok && readExact(fd, &parsed.spoofBuild, sizeof(parsed.spoofBuild));
        ...
        ok = ok && readString(fd, parsed.fingerprint);
        ok = ok && readString(fd, parsed.model);
        ok = ok && readString(fd, parsed.product);
        ok = ok && readString(fd, parsed.brand);
        ok = ok && readString(fd, parsed.device);
        ok = ok && readString(fd, parsed.manufacturer);
